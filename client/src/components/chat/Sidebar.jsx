// 1. DÜZELTME: useState'i import ettik
import React, { useState } from "react"; 
import { useNavigate } from "react-router-dom"; 

// 2. DÜZELTME: Modalı import ettik (Dosya yoluna dikkat)
import AddFriendModal from "./AddFriendModal";

const Sidebar = ({ currentUser, users, onlineUsers, selectedUser, onSelectUser, onLogout, onFriendAdded  }) => {
  ...
  return (
    <div className="w-1/3 bg-white border-r border-gray-300 flex flex-col h-full relative">
      {/* --- MODAL (Buraya ekledik) --- */}
      {/* isModalOpen true olunca ekranda belirir */}
      <AddFriendModal
        isOpen={isModalOpen}
        onClose={() => setIsModalOpen(false)}
        onFriendAdded={onFriendAdded} // <--- İŞTE BU
      />
      ...
      <div className="p-4 bg-gray-50 border-b flex justify-between items-center shrink-0 shadow-sm z-10">
        ...
        <div className="flex items-center gap-2">
          {/* 3. DÜZELTME: ARKADAŞ EKLE (+) BUTONU */}
          <button
