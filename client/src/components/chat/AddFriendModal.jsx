...
export default function AddFriendModal({ isOpen, onClose, onFriendAdded }) {
  ...
  const loadRequests = async () => {
    try {
      // DÜZELTME 1: Servisteki doğru isim 'getPendingRequests'
  ...
  const sendRequest = async (userId) => {
    try {
      // DÜZELTME 2: Servisteki doğru isim 'sendFriendRequest'
      ...
      console.error(error); // Hatayı konsolda görmek için
  ...
  const acceptRequest = async (requestId, senderId) => {
    try {
      // DÜZELTME 3: Servisteki doğru isim 'acceptFriendRequest'
        ...
        <div className="flex border-b">
          <button
            className={`flex-1 py-3 font-medium ${
              activeTab === "search" ? "text-blue-600 border-b-2 border-blue-600" : "text-gray-500 hover:bg-gray-50"
            }`}
          ...
          <button
            className={`flex-1 py-3 font-medium ${
              activeTab === "requests" ? "text-blue-600 border-b-2 border-blue-600" : "text-gray-500 hover:bg-gray-50"
            }`}
            onClick={() => setActiveTab("requests")}
          >
            Gelen İstekler{" "}
            {requests.length > 0 && <span className="bg-red-500 text-white text-xs px-2 py-0.5 rounded-full ml-1">{requests.length}</span>}
          </button>
        </div>
                    ...
                    <div className="flex items-center gap-3">
                      <img
                        src={
                          ...
                          req.sender.avatar ||
                          `https://api.dicebear.com/9.x/avataaars/svg?seed=${req.sender.userName}`
                        }
